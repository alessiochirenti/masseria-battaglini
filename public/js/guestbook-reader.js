...
export function createGuestbookReader(pages) {
  ...
  dialog.innerHTML = `
    <header class="guestbook-reader__header">
      <h2 id="${id}-heading">${t('Il libro degli ospiti')}</h2>
      <button class="guestbook-reader__button guestbook-reader__close" type="button" aria-label="${t('Chiudi il libro degli ospiti')}">${t('Chiudi')}<span aria-hidden="true" class="guestbook-reader__cross"></span></button>
    </header>
    <div class="guestbook-reader__body">
      <div class="guestbook-reader__viewport" tabindex="0" role="region" aria-label="${t('Pagina del libro degli ospiti')}" aria-describedby="${id}-help" data-lenis-prevent>
        ...
        <div class="guestbook-reader__feedback">
          ...
          <button class="guestbook-reader__button guestbook-reader__retry" type="button" hidden>${t('Riprova')}</button>
        </div>
      </div>
      <details class="guestbook-reader__transcription">
        <summary>${t('Leggi il testo')}<span class="guestbook-reader__disclosure" aria-hidden="true"></span></summary>
    ...
    <footer class="guestbook-reader__footer">
      <nav class="guestbook-reader__navigation" aria-label="${t('Pagine del libro degli ospiti')}">
        <button class="guestbook-reader__button guestbook-reader__previous" type="button" aria-label="${t('Pagina precedente')}"><span class="guestbook-reader__arrow guestbook-reader__arrow--previous" aria-hidden="true"></span><span class="guestbook-reader__direction">${t('Precedente')}</span></button>
        <p class="guestbook-reader__counter" aria-live="polite" aria-atomic="true"></p>
        <button class="guestbook-reader__button guestbook-reader__next" type="button" aria-label="${t('Pagina successiva')}"><span class="guestbook-reader__direction">${t('Successiva')}</span><span class="guestbook-reader__arrow" aria-hidden="true"></span></button>
      </nav>
      <button class="guestbook-reader__button guestbook-reader__zoom" type="button" aria-pressed="false">${t('Ingrandisci')}</button>
    </footer>
    <p class="guestbook-reader__sr-only" id="${id}-help">${t('Usa le frecce sinistra e destra per cambiare pagina. Ingrandisci per scorrere la scansione nei dettagli. Premi Esc per chiudere.')}</p>`
  ...
  function setZoom(value) {
    ...
    zoom.textContent = value ? t('Riduci') : t('Ingrandisci')
  ...
  function loadPage(index) {
    ...
    const title = entry.title || `${t('Pagina')} ${current + 1}`
    setZoom(false)
    transcription.open = false
    transcription.hidden = !entry.transcript
    pageTitle.textContent = title
    transcript.textContent = entry.transcript || ''
    transcript.lang = entry.lang || document.documentElement.lang
    select('text').scrollTop = 0
    counter.textContent = `${current + 1} / ${entries.length}`
    counter.setAttribute('aria-label', t('Pagina %n% di %total%', {n:current+1,total:entries.length}))
    previous.disabled = current === 0
    next.disabled = current === entries.length - 1
    zoom.disabled = true
    retry.hidden = true
    status.textContent = t('Caricamento pagina…')
    ...
    scan.onload = () => {
      ...
      image.alt = `${t('Scansione originale del libro degli ospiti.')} ${title}`
      image.src = scan.src
    ...
    scan.onerror = () => {
      ...
      status.textContent = entry.transcript
        ? t('La pagina non si è caricata. Puoi riprovare o leggere il testo qui sotto.')
        : t('La pagina non si è caricata. Puoi riprovare.')
      retry.hidden = false
    }
